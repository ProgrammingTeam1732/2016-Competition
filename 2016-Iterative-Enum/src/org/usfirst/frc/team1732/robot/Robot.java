...
public class Robot extends IterativeRobot {
	...
	StateMachine<cross_terrain_sm_enum> cross_terrain = new StateMachine<cross_terrain_sm_enum>();
	public enum cross_terrain_sm_enum {
		ArmDown, DriveAcc, DriveSteady, Finished;
	}
	
	StateMachine portcullis_sm = new StateMachine();
	public enum portcullis_sm_enum {

	}
	
	StateMachine cheval_sm = new StateMachine();
	public enum cheval_sm_enum {

	}
	
	StateMachine drawbridge_sm = new StateMachine();
	public enum drawbridge_sm_enum {

	}
	
	StateMachine sally_port_sm = new StateMachine();
	public enum sally_port_sm_enum {
	...
	public void teleopPeriodic() {
		if (sm.getState().equals("Auto Init"))
			sm.setState(sm_enum.WaitToShoot);
		if (sm.getState().equals("Point at Goal"))
			bot.run(sm.process(bot.getCameraState()), input);
		else
			bot.run(sm.process(bot.getState(input.getShoot(), input.getResetShot())), input);
