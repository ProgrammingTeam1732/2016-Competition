...
public class Arm {
	...
	private AnalogInput	pot		= new AnalogInput(2);	// 3 on practice, 2 on
														// competition bot
	...
	static int	High	= (3340);			// 3135 practice, 3340 competition,
											// 3120 was old competition
	...
	public Arm() {
		...
		if (inDeadbandLow())
			setpoint = Low;
		else if (inDeadbandMiddle())
			setpoint = Middle;
		else if (inDeadbandHigh())
			setpoint = High;
		else if (inDeadbandAuto())
			setpoint = Auto;
		else if (inDeadbandCheval())
			setpoint = Cheval;
	}

	public void run() {
		/*
		 * if (!auto) { if (dir == -1) { motor.set(DOWN_SPEED); } else if (dir
		 * == 1) { motor.set(UP_SPEED); } else if (dir == 0) { motor.set(STOP);
		 * } else { System.err.println("Unknown Arm Direction");
		 * motor.set(STOP); } } else {
		 */
	...
	/*
	 * public void setUp() { auto = false; dir = 1; }
	 * 
	 * public void setDown() { auto = false; dir = -1; }
	 * 
	 * public void stop() { auto = false; dir = 0; }
	 */
