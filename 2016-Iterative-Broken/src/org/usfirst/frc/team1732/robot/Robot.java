	...
	public void robotInit() {
		...
		*/
		...
		sm.addState(new State("Wait to Shoot", (RobotState rbs) -> {
			...
			if (rbs.shoot_mode_far || rbs.shoot_mode_close) return "Shoot Position";
			else if (!rbs.camera_exists) return "Wait to Shoot";
			else if (rbs.shoot_mode_auto) return "Point at Goal";
			else return null;
		...
		low_bar_sm.addState(new State("Drop Intake", (RobotState rbs) -> {
			...
			if (((rbs.drive_left_dist + rbs.drive_left_dist) / 2.0) * 0.095 > distance) {
				return "Raise Intake";
			}
			return null;
		})).addState(new State("Raise Intake", (RobotState rbs) -> {
			RobotInstruction rbi = new RobotInstruction();
			rbi.intake_up = true;
			rbi.reset_drive = true;
			return rbi;
		}, (RobotState rbs) -> {
			if(rbs.intake_up && System.currentTimeMillis() - rbs.start_time > 250) return "Drive Curve";
			else return null;
		})).addState(new State("Drive Curve", (RobotState rbs) -> {
			RobotInstruction rbi = new RobotInstruction();
			rbi.drive_left = 1*scalar;
			rbi.drive_right = ratio*scalar;
			return rbi;
		}, (RobotState rbs) -> {
			if(rbs.drive_left_dist > left || rbs.drive_right_dist > right) return "Lower Intake";
			return null;
		})).addState(new State("Lower Intake", (RobotState rbs) -> {
			RobotInstruction rbi = new RobotInstruction();
			rbi.intake_down = true;
			return rbi;
		}, (RobotState rbs) -> {
			if(rbs.intake_up && System.currentTimeMillis() - rbs.start_time > 250) return "Raise Arm";
			else return null;
	...
	public void autonomousPeriodic() {
		if (start_mode.equals(default_auto)) {
			...
			if (!over) {
				...
				bot.run(sm.process(bot.getState())); // shoot, will not do anything after shooting
			} else {
